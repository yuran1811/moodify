...
const notifyTemplate = () => `
  <div class="e-card playing" role="draggable" style="top:52px;left:52px">
    ...
    <div class="infotop">
      ...
      <br />
      <div class="name">Powered by <a target="_blank" href="https://dtrung98.github.io/GPABookmarklet">dtrung98</a></div>

      <br />
      <div class="name" style="padding:0 32px">Drag and drop the button below to the bookmark bar</div>
      <a href="${window?.encodedMoodifyGPACalc ? window.encodedMoodifyGPACalc : ""}">
      <button role="GPACalc" class="btn-17">
        <span class="text-container">
        <span class="text">Calculate GPA</span>
        </span>
      </button>
      </a>
...
chrome.storage.sync.get(["portalGPACalculate"], (data) => {
  if (!data?.portalGPACalculate) return;
  ...
  $('button[role="GPACalc"').on("click", (e) => {
    e.preventDefault();
    e.stopPropagation();

    // window?.MoodifyGPACalc && window.MoodifyGPACalc();
  });

  $('button[role="GPACalc"').on("mousemove", (e) => {
    e.preventDefault();
    e.stopPropagation();
  });
});
