async function GPACalc() {
  ...
  var param = dataArray.reduce(function (a, x) {
    a[x.name] = x.value;
    return a;
  }, {});
  ...
  const isAllSection =
    param[
      "ctl00$ContentPlaceHolder1$ctl00$cboNamHoc_gvDKHPLichThi$ob_CbocboNamHoc_gvDKHPLichThiTB"
    ] == "--Tất cả--";
  let semeterInput = $(
    'input[name="ctl00$ContentPlaceHolder1$ctl00$cboHocKy_gvDKHPLichThi$ob_CbocboHocKy_gvDKHPLichThiTB"]',
  );
  ...
  function goToAllCoursesPage() {
    ...
    param[
      "ctl00$ContentPlaceHolder1$ctl00$cboNamHoc_gvDKHPLichThi$ob_CbocboNamHoc_gvDKHPLichThiSIS"
    ] = "0";
    param["ctl00$ContentPlaceHolder1$ctl00$cboNamHoc_gvDKHPLichThi"] = "0";
    param[
      "ctl00$ContentPlaceHolder1$ctl00$cboHocKy_gvDKHPLichThi$ob_CbocboHocKy_gvDKHPLichThiTB"
    ] = "0";
    param[
      "ctl00$ContentPlaceHolder1$ctl00$cboHocKy_gvDKHPLichThi$ob_CbocboHocKy_gvDKHPLichThiSIS"
    ] = "0";
    ...
    param["ctl00$ContentPlaceHolder1$ctl00$btnXemDiemThi"] =
      "Xem Kết Quả Học Tập";
    ...
    if (parentDiv && !$("#loading-text")[0])
      parentDiv.prepend(
        $(
          '<div id="loading-text" style="font-size:20px; color:#1B486A;text-align:center;margin:10px;">Chờ một chút...</div>',
        )[0],
      );
    ...
    return new Promise((resolve, reject) => {
      $.ajax({
        ...
        success: function (res) {
          ...
          let HK = $(html).find(
            "input[name='ctl00$ContentPlaceHolder1$ctl00$cboNamHoc_gvDKHPLichThi$ob_CbocboNamHoc_gvDKHPLichThiTB']",
          );

          if (HK) console.log($(HK[0]).attr("value"));
  ...
  function addSupplementaryGrade() {
    ...
    data.forEach((item) => {
      if (item.score >= 0) {
        const lowerGrade = supplementaryGrade.find(
          (grade) => item.score >= grade.score,
        );
        const upperGrade = supplementaryGrade.find(
          (grade) => item.score < grade.score,
        );
        ...
        if (lowerGrade && upperGrade) {
          ...
          item.fourRounding = toFixed(
            lowerGrade.fourRounding +
              ratio * (upperGrade.fourRounding - lowerGrade.fourRounding),
          );
        } else {
  ...
  class Calculation {
    ...
    calculateGPA() {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (item.include) {
          ...
          if (item.whyExclude.includes("5")) {
            // Used to calculate even not passed gpa
      ...
      this.notPassGPA =
        (this.totalScores + this.notPassTotalScore) /
        (this.totalCredits + this.notPassCredits);
      ...
      console.log(
        "%c \n Tổng học phần trong GPA:\n %c" +
          (data.length - this.removedCoursesSize) +
          "\n",
        "color:black",
        "color:blue; font-size: 30px;",
      );
      ...
      for (let i = 0; i < exceptData.length; i++) {
        removedSection +=
          ...
          exceptData[i].course +
          " (" +
          exceptData[i].semester +
          ")" +
          "\n%c lý do: %c" +
          exceptData[i].whyExclude +
          "\n\n%c";
      ...
      removedSection +=
        "%c  " + this.removedCoursesSize + " học phần đã loại bỏ.%c\n";
      removedCss.push("color:red");
      removedCss.push("color:black");
    ...
    formatCoursesTableAndCreateResultTable() {
      ...
      if (isRunBookmarkAgain == false) {
        // First time calculate GPA. Insert checkbox column.
        ...
        for (let i = 0; i < rows.length; i++) {
          $(rows[i]).prepend(
            '<td class= "center gpa-checkbox" style="width:60px;" ><input type="checkbox"' +
              (data[i].include ? " checked " : "") +
              " />" +
              "<div hidden>" +
              (data[i].include ? 1 : 0) +
              "</div></td>",
          ); // Use hidden div to enable sorting checkbox column
        }
      }
      ...
      $(gpaTableBody).append(
        '<tr class="odd"><td class="left ">Điểm trung bình tích lũy (GPA)</td><td class="center gpa" id="calGPA"><b>' +
          toFixed(this.gpa) +
          "</b></td></tr>",
      );
      $(gpaTableBody).append(
        '<tr class="odd"><td class="left ">Điểm trung bình tích lũy (GPA) hệ 4</td><td class="center gpa" id="calGPA"><b>' +
          toFixed(this.fourGPA) +
          "</b></td></tr>",
      );
      $(gpaTableBody).append(
        '<tr class="odd"><td class="left ">Điểm trung bình học tập</td><td class="center gpa" id="calGPA">' +
          toFixed(this.notPassGPA) +
          "</td></tr>",
      );
      $(gpaTableBody).append(
        '<tr class="even"><td class="left">Tổng tín chỉ đã tích luỹ</td><td class="center gpa" id="calSumCredit">' +
          this.totalCredits +
          " tín chỉ</td></tr>",
      );
      $(gpaTableBody).append(
        '<tr class="odd"><td class="left">Tổng điểm đã tích lũy</td><td class="center gpa" id="sumScore">' +
          this.totalScores +
          "</td></tr>",
      );
      $(gpaTableBody).append(
        '<tr class="even"><td class="left">Sô học phần đã học</td><td class="center gpa" id="sumCourse">' +
          data.length +
          " học phần</td></tr>",
      );
      $(gpaTableBody).append(
        '<tr class="odd"><td class="left">Số học phần tính trong GPA</td><td class="center gpa" id="sumCalCourse">' +
          (data.length - this.removedCoursesSize) +
          " học phần</td></tr>",
      );
      ...
      for (let i = 0; i < rows.length; i++) {
        if (!data[i].include) {
          ...
          if (data[i].whyExclude == "") $(rows[i]).removeAttr("style");
            ...
            $(rows[i]).attr(
              "style",
              "color:yellow;text-decoration: line-through;",
            );
        }
      }
    }
  ...
  class SaveCoursesList {
    ...
    saveToFileCSV() {
      let idxRow = -1;
      let csv =
        "Tên môn học, Số tín chỉ, Điểm, Điểm chữ, Điểm 4, Học kỳ, Lớp, Ghi chú\n";
      ...
      csv +=
        "Tổng học phần trong GPA: " +
        (this.data.length - this.cal.removedCoursesSize) +
        "\n";
  ...
  tab.dataTable({
    ...
    bAutoWidth: false,
    aaSorting: [[1, "asc"]],
    aoColumnDefs: [{ sWidth: "50%", aTargets: [2] }],
  });
  ...
  $("#tbDiemThiGK").on("change", 'input[type="checkbox"]', function () {
    ...
    $(this)
      .siblings()
      .first()
      .text($(this).is(":checked") ? 1 : 0); // Change value of hidden div for sorting
    $(htmlCheckBox).attr("checked", $(this).is(":checked"));

    tab.fnUpdate(
      courseRow.children().first().html(),
      courseRow[0],
      0,
      false,
      false,
    ); // Update html of checkbox in DataTable
  ...
  if (!$("#saveCoursesList")[0]) {
    ...
    saveButton = $("#ob_iBbtnXemDiemThiContainer")
      .clone()
      .attr("id", "saveCoursesList");
  ...
  if (!$("#isCalSemester")[0]) {
    let checkbox = $(
      '<input type="checkbox" id="isCalSemester"><label for="isCalSemester"> Tính theo học kỳ</label><br><br>',
    );
